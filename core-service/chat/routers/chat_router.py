...
def get_chat_rooms(user_id: str, db: Session = Depends(get_db)):
    ...
    for r in rooms:
        unread = get_unread_count(db, room_id=r["room_id"], user_id=user_id)
        ...
        existing = conn.execute(
            ...
            .where(
                or_(
                    ...
                    and_(chat_rooms.c.author_id == author_id, 
                        chat_rooms.c.peer_id == peer_id),
                    # Case 2: B(author), A(peer) 순서가 뒤바뀌어 방이 이미 존재할 때
                    and_(chat_rooms.c.author_id == peer_id, 
                        chat_rooms.c.peer_id == author_id), 
                )
            )  
        ).mappings().first()
